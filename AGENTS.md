...
Surfaced is a Firefox WebExtension (Manifest V3) that tracks cumulative infinite-scroll depth and shows gentle notifications when the user has gone too far down a page.
It targets Firefox desktop and Firefox for Android, keeps all data local in the browser, and uses plain vanilla JavaScript/HTML/CSS with no external runtime dependencies.
...
- Shared browsing behavior lives in `shared/` and must remain compatible with both desktop and Android builds.
- The unified AMO package is the primary release artifact; `android/manifest.json` is the source of truth for the unified manifest and version.
...
- Prefer changing `shared/` when behavior should match across desktop and Android. Do not fork logic into platform directories without a clear UI or manifest reason.
...
- If you bump the extension version for release work, update `android/manifest.json` first and keep `desktop/manifest.json` in sync.
